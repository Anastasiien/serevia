...
//  serevia
//
//  Created by ekatizzz on 06.04.2026.
//
...
// Константы палитры для удобства применения
struct AppColors {
    static let background = UIColor(red: 0.96, green: 0.94, blue: 0.91, alpha: 1.0)
    static let primary = UIColor(red: 0.58, green: 0.46, blue: 0.42, alpha: 1.0)
    static let secondary = UIColor(red: 0.67, green: 0.55, blue: 0.42, alpha: 1.0)
    static let accent = UIColor(red: 0.82, green: 0.71, blue: 0.55, alpha: 1.0)
    static let text = UIColor(red: 0.25, green: 0.20, blue: 0.15, alpha: 1.0)
    static let lightText = UIColor(red: 0.50, green: 0.44, blue: 0.41, alpha: 1.0)
    static let card = UIColor(red: 0.98, green: 0.97, blue: 0.96, alpha: 1.0)
    static let border = UIColor(red: 0.82, green: 0.71, blue: 0.55, alpha: 0.3)
    static let sectionBackground = UIColor(red: 0.90, green: 0.85, blue: 0.78, alpha: 1.0)
}
...
class MeditateViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        durationSlider.tintColor = AppColors.secondary
        ...
        let divider = UIView()
        divider.backgroundColor = AppColors.border.withAlphaComponent(0.1)
    ...
    private func startTimerResume() {
        timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
            ...
            if self.remainingSeconds > 0 {
                ...
                self.timerLabel.text = "✅ Завершено"
    ...
    private func setupCategoryButtons() {
        ...
        for cat in categories {
            ...
            button.setTitleColor(isActive ? .white : AppColors.primary, for: .normal)
    ...
    private func handleMeditationPlay(meditation: (name: String, duration: Int), playButton: UIButton, cancelButton: UIButton, durationLabel: UILabel) {
        stopSound()
    ...
    private func startMeditationTimer(playButton: UIButton, cancelButton: UIButton, durationLabel: UILabel) {
    ...
    private func cancelMeditation(playButton: UIButton, cancelButton: UIButton, durationLabel: UILabel, duration: Int) {
        activeMeditationTimer?.invalidate()
        activeMeditationTimer = nil
        playButton.setTitle("▶️", for: .normal)
        cancelButton.isHidden = true
    ...
    private func updateMeditations() {
        ...
        if selectedCategory == "Все" { items = meditations.values.flatMap { $0 } }
        else if selectedCategory == "Избранное" {
            items = Array(favorites).compactMap { name in
                for category in meditations.values { if let item = category.first(where: { $0.name == name }) { return item } }
                return nil
            }
        } else { items = meditations[selectedCategory] ?? [] }
        ...
        for meditation in items {
            ...
            playButton.addAction(UIAction(handler: { [weak self] _ in
                self?.handleMeditationPlay(meditation: meditation, playButton: playButton, cancelButton: cancelButton, durationLabel: durationLabel)
            }), for: .touchUpInside)

            cancelButton.addAction(UIAction(handler: { [weak self] _ in
                self?.cancelMeditation(playButton: playButton, cancelButton: cancelButton, durationLabel: durationLabel, duration: meditation.duration)
            }), for: .touchUpInside)
