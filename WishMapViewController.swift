...
class WishMapEditorViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIGestureRecognizerDelegate {
    ...
    private func setupCanvas() {
        ...
        NSLayoutConstraint.activate([
            ...
            // соотношение 4:3 горизонтальное — как карточка на главном экране
            canvasView.heightAnchor.constraint(equalTo: canvasView.widthAnchor, multiplier: 3.0/4.0),
    ...
    @objc private func clearCanvas() {
        ...
        alert.addAction(UIAlertAction(title: "Удалить", style: .destructive) { _ in
            ...
            self.hideTools()
            UserDefaults.standard.removeObject(forKey: "wishMapImage")
            NotificationCenter.default.post(name: .wishMapDidUpdate, object: nil)
        })
    ...
    private func loadExistingMap() {
        guard let data = UserDefaults.standard.data(forKey: "wishMapImage"),
              let image = UIImage(data: data) else { return }
        let iv = UIImageView(image: image)
        iv.contentMode = .scaleAspectFill
        iv.clipsToBounds = true
        iv.frame = canvasView.bounds
        iv.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        iv.tag = 999
        canvasView.insertSubview(iv, at: 0)
    }
    ...
    @objc private func saveToHome() {
        ...
        if let data = image.jpegData(compressionQuality: 0.85) {
            UserDefaults.standard.set(data, forKey: "wishMapImage")
        }
        NotificationCenter.default.post(name: .wishMapDidUpdate, object: nil)

        let ac = UIAlertController(title: "Сохранено ✓", message: "Карта желаний появится на главном экране", preferredStyle: .alert)
        ac.addAction(UIAlertAction(title: "Отлично", style: .default) { [weak self] _ in
